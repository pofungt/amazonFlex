  ...
  def __init__(self) -> None:
    try:
      with open("config.json") as configFile:
        ...
        self.tgEnabled = config["tg_enabled"]
        self.tgToken = config["tg_token"]
        self.tgChatId = config["tg_chat_id"]
  ...
  def run(self):
    ...
    while self.__retryCount < self.retryLimit:
      if not self.__retryCount % 5:
          message = f"{self.__retryCount} requests attempted"
          print(message + '\n\n')
          if self.tgEnabled:
            send_telegram_message(self.tgChatId, self.tgToken, message)
